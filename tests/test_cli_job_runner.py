...
def test_job_run() -> None:
	...
	commands = [ sys.executable, 'facefusion.py', 'job-add-step', 'test-job-run', '--jobs-path', get_test_jobs_directory(), '--processors', 'face_debugger', '--target-path', get_test_example_file('target-240p.jpg'), '--output-path', get_test_output_file('test-job-run.jpg') ]
	subprocess.run(commands)
...
def test_job_run_all() -> None:
	...
	commands = [ sys.executable, 'facefusion.py', 'job-add-step', 'test-job-run-all-1', '--jobs-path', get_test_jobs_directory(), '--processors', 'face_debugger', '--target-path', get_test_example_file('target-240p.jpg'), '--output-path', get_test_output_file('test-job-run-all-1.jpg') ]
	subprocess.run(commands)

	commands = [ sys.executable, 'facefusion.py', 'job-add-step', 'test-job-run-all-2', '--jobs-path', get_test_jobs_directory(), '--processors', 'face_debugger', '--target-path', get_test_example_file('target-240p.mp4'), '--output-path', get_test_output_file('test-job-run-all-2.mp4'), '--trim-frame-end', '1' ]
	subprocess.run(commands)

	commands = [ sys.executable, 'facefusion.py', 'job-add-step', 'test-job-run-all-2', '--jobs-path', get_test_jobs_directory(), '--processors', 'face_debugger', '--target-path', get_test_example_file('target-240p.mp4'), '--output-path', get_test_output_file('test-job-run-all-2.mp4'), '--trim-frame-start', '0', '--trim-frame-end', '1' ]
	subprocess.run(commands)
...
def test_job_retry() -> None:
	...
	commands = [ sys.executable, 'facefusion.py', 'job-add-step', 'test-job-retry', '--jobs-path', get_test_jobs_directory(), '--processors', 'face_debugger', '--target-path', get_test_example_file('target-240p.jpg'), '--output-path', get_test_output_file('test-job-retry.jpg') ]
	subprocess.run(commands)
...
def test_job_retry_all() -> None:
	...
	commands = [ sys.executable, 'facefusion.py', 'job-add-step', 'test-job-retry-all-1', '--jobs-path', get_test_jobs_directory(), '--processors', 'face_debugger', '--target-path', get_test_example_file('target-240p.jpg'), '--output-path', get_test_output_file('test-job-retry-all-1.jpg') ]
	subprocess.run(commands)

	commands = [ sys.executable, 'facefusion.py', 'job-add-step', 'test-job-retry-all-2', '--jobs-path', get_test_jobs_directory(), '--processors', 'face_debugger', '--target-path', get_test_example_file('target-240p.mp4'), '--output-path', get_test_output_file('test-job-retry-all-2.mp4'), '--trim-frame-end', '1' ]
	subprocess.run(commands)

	commands = [ sys.executable, 'facefusion.py', 'job-add-step', 'test-job-retry-all-2', '--jobs-path', get_test_jobs_directory(), '--processors', 'face_debugger', '--target-path', get_test_example_file('target-240p.mp4'), '--output-path', get_test_output_file('test-job-retry-all-2.mp4'), '--trim-frame-start', '0', '--trim-frame-end', '1' ]
	subprocess.run(commands)
