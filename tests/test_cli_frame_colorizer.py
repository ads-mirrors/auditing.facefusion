...
def test_colorize_frame_to_image() -> None:
	commands = [ sys.executable, 'facefusion.py', 'headless-run', '--jobs-path', get_test_jobs_directory(), '--processors', 'frame_colorizer', '--target-path', get_test_example_file('target-240p-0sat.jpg'), '--output-path', get_test_output_file('test_colorize-frame-to-image.jpg') ]
...
def test_colorize_frame_to_video() -> None:
	commands = [ sys.executable, 'facefusion.py', 'headless-run', '--jobs-path', get_test_jobs_directory(), '--processors', 'frame_colorizer', '--target-path', get_test_example_file('target-240p-0sat.mp4'), '--output-path', get_test_output_file('test-colorize-frame-to-video.mp4'), '--trim-frame-end', '1' ]
